...
class TD3Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.conv2 = nn.Conv1d(5,25,3)
        self.conv3 = nn.Conv1d(25,125,3)
        self.conv4 = nn.Conv1d(125,25,3)
        self.linear1 = nn.Linear(self.feature_size(state_dim),256)
    ...
    def forward(self, state):
        if (len(state.shape)==2):
            state = state.view(state.shape[0],1,state.shape[1])
        ...
        # a = F.relu(self.l2(a))
        # a = self.dropout2(a)
        return self.max_action * torch.tanh(self.linear2(a))
    
    def feature_size(self, input_shape):
        return self.conv4(self.conv3(self.conv2(self.conv1(torch.zeros(2,1,input_shape))))).view(2, -1).size(-1)
...
class TD3Trainer:
    ...
    def train(self, replay_buffer, batch_size=256):
        ...
        with torch.no_grad():
            ...
            Q_intermediate = torch.min(Q1_target, Q2_target)
