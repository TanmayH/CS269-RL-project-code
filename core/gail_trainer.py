...
class GAILTrainer(PPOTrainer):
    ...
    def compute_loss(self, sample):
        ...
        surrogate_1 = ratio * gail_rewards
        surrogate_2 = torch.clamp(ratio, 1.0 - self.clip_param, 1.0 + self.clip_param) * gail_rewards
    ...
    def update(self, rollout):
        ...
        for epoch in range(1, self.config.discriminator_epoch + 1):  # Run multiple epochs
            ...
            for (expert_data, agent_data) in zip(self.expert_dataloader, data_generator):
                ...
                # print(agent_generated_obs.shape, agent_generated_actions.shape)
                # print(expert_generated_obs.shape, expert_generated_actions.shape)
                ...
                discriminator_loss = discriminator_loss_func(agent_prediction, torch.zeros(agent_prediction.shape)) + discriminator_loss_func(expert_prediction, torch.ones(expert_prediction.shape))
