...
class TD3Actor(nn.Module):
    ...
    def forward(self, state):
        a = self.bn1(F.leaky_relu(self.l1(state)))
        # a = self.dropout1(a)
        a = self.bn2(F.leaky_relu(self.l2(a)))
        # a = self.dropout2(a)
...
class TD3Trainer:
    ...
    def train(self, replay_buffer, batch_size=256, noise_reduction=0):
        self.total_it += 1
        ...
        with torch.no_grad():
            ...
            # Normal ditribution with STD self.policy_noise.
            noise = (
                    torch.randn_like(action) * (self.policy_noise-noise_reduction)
            ...
            Q_intermediate = torch.min(Q1_target, Q2_target) + 0.01*(torch.max(Q1_target, Q2_target)-torch.min(Q1_target, Q2_target))
            # Q_intermediate = 0.95*torch.min(Q1_target, Q2_target) + 0.05*torch.max(Q1_target, Q2_target)
            target_Q = reward + not_done * self.discount * Q_intermediate
    ...
    def load(self, filename):
        self.critic.load_state_dict(torch.load(os.path.join(filename, "critic"),map_location = device))
        self.critic_optimizer.load_state_dict(torch.load(os.path.join(filename, "critic_optimizer"),map_location = device))
        self.critic_target = copy.deepcopy(self.critic)

        self.actor.load_state_dict(torch.load(os.path.join(filename, "actor"),map_location = device))
        self.actor_optimizer.load_state_dict(torch.load(os.path.join(filename, "actor_optimizer"),map_location = device))
        self.actor_target = copy.deepcopy(self.actor)
        return True
